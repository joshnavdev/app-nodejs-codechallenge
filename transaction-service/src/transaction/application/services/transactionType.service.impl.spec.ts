...
describe('TransactionTypeServiceImpl', () => {
  ...
  it('should throw an error if transaction type not found', async () => {
    ...
    await expect(service.findOneByName(transactionTypeName)).rejects.toThrow(BadRequestError);
    await expect(service.findOneByName(transactionTypeName)).rejects.toThrow('Transaction type not found');
    expect(transactionTypeRepo.findOneByName).toHaveBeenCalledWith(transactionTypeName);
  });
});
