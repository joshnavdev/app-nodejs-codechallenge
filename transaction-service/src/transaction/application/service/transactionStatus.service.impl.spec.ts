...
describe('TransactionStatusServiceImpl', () => {
  ...
  it('should throw an error if transaction status not found', async () => {
    ...
    await expect(service.findOneByName(transactionStatusName)).rejects.toThrow(RpcBusinessException);
    await expect(service.findOneByName(transactionStatusName)).rejects.toThrow('Transfer type not found');
    expect(transactionStatusRepo.findOneByName).toHaveBeenCalledWith(transactionStatusName);
  });
});
